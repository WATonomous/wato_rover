...
class Transformer(Node):
    def __init__(self):
        ...
        self.__buffer_capacity = (
            self.get_parameter("buffer_capacity").get_parameter_value().integer_value
        )
    ...
    def unfiltered_callback(self, msg):
        ...
        filtered_msg.pos_x, filtered_msg.pos_y, filtered_msg.pos_z = (
            self.__transformer.deserialize_data(msg.data)
        )
        filtered_msg.timestamp = msg.timestamp
        filtered_msg.metadata.version = (
            self.get_parameter("version").get_parameter_value().integer_value
        )
        filtered_msg.metadata.compression_method = (
            self.get_parameter("compression_method").get_parameter_value().integer_value
        )
