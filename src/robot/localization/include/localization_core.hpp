...
public:
    ...
    double& operator()(int i, int j) { return data_[i * 6 + j]; }
    const double& operator()(int i, int j) const { return data_[i * 6 + j]; }
...
public:
    ...
    double omega() const { return data_[5]; }
...
public:
    ...
    void predict(double vx, double vy, double omega, double dt,
                 double process_noise_pos, double process_noise_theta, double process_noise_vel);
    ...
    void getOdometry(nav_msgs::msg::Odometry& odom_msg,
