    ...
    initialized_(false)
{
    ...
    imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic_,
        10,
        std::bind(&ImuSimNode::imuCallback, this, std::placeholders::_1)
    );
...
void ImuSimNode::imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
    ...
    imu_sim_msg.orientation.x = msg->orientation.x + 
                                 generateGaussianNoise(0.0, orientation_noise_std_) +
                                 (add_bias_ ? orientation_bias_x_ : 0.0);
    imu_sim_msg.orientation.y = msg->orientation.y + 
                                 generateGaussianNoise(0.0, orientation_noise_std_) +
                                 (add_bias_ ? orientation_bias_y_ : 0.0);
    imu_sim_msg.orientation.z = msg->orientation.z + 
                                 generateGaussianNoise(0.0, orientation_noise_std_) +
                                 (add_bias_ ? orientation_bias_z_ : 0.0);
    imu_sim_msg.orientation.w = msg->orientation.w + 
                                 generateGaussianNoise(0.0, orientation_noise_std_) +
                                 (add_bias_ ? orientation_bias_w_ : 0.0);

    // Normalize quaternion (important for valid orientation)
    double norm = std::sqrt(imu_sim_msg.orientation.x * imu_sim_msg.orientation.x +
                           imu_sim_msg.orientation.y * imu_sim_msg.orientation.y +
                           imu_sim_msg.orientation.z * imu_sim_msg.orientation.z +
                           imu_sim_msg.orientation.w * imu_sim_msg.orientation.w);
    if (norm > 0.0) {
    ...
    imu_sim_msg.angular_velocity.x = msg->angular_velocity.x + 
                                      generateGaussianNoise(0.0, angular_vel_noise_std_) +
                                      (add_bias_ ? angular_vel_bias_x_ : 0.0);
    imu_sim_msg.angular_velocity.y = msg->angular_velocity.y + 
                                      generateGaussianNoise(0.0, angular_vel_noise_std_) +
                                      (add_bias_ ? angular_vel_bias_y_ : 0.0);
    imu_sim_msg.angular_velocity.z = msg->angular_velocity.z + 
                                      generateGaussianNoise(0.0, angular_vel_noise_std_) +
                                      (add_bias_ ? angular_vel_bias_z_ : 0.0);

    // Add noise and bias to linear acceleration
    imu_sim_msg.linear_acceleration.x = msg->linear_acceleration.x + 
                                        generateGaussianNoise(0.0, linear_accel_noise_std_) +
                                        (add_bias_ ? linear_accel_bias_x_ : 0.0);
    imu_sim_msg.linear_acceleration.y = msg->linear_acceleration.y + 
                                        generateGaussianNoise(0.0, linear_accel_noise_std_) +
                                        (add_bias_ ? linear_accel_bias_y_ : 0.0);
    imu_sim_msg.linear_acceleration.z = msg->linear_acceleration.z + 
                                        generateGaussianNoise(0.0, linear_accel_noise_std_) +
