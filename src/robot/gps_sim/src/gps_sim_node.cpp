...
, gaussian_noise_(0.0, 1.0)
{
  ...
  timer_ = this->create_wall_timer(std::chrono::milliseconds(period_ms), std::bind(&GpsSimNode::timerCallback, this));
...
void GpsSimNode::timerCallback()
{
  ...
  if (simulate_dropouts_) {
    ...
    if (random_value < gps_dropout_probability_) {
      RCLCPP_DEBUG_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "GPS dropout simulated - not publishing fix");
      return;
    }
  }
