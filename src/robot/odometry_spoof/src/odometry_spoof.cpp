...
: Node("odometry_spoof")
{
  ...
  timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&OdometrySpoofNode::timerCallback, this));
}

void OdometrySpoofNode::timerCallback()
{
  // We'll look up the transform from sim_world -> robot/chassis/camera,
  // note robot frame is usually not the camera sensor, but we do so to make this
  // assignment easier
  const std::string target_frame = "robot/chassis/camera";
  ...
  if (has_last_transform_) {
    ...
    if (dt > 0.0) {
      ...
      tf2::Quaternion q_last(
        last_orientation_.x(), last_orientation_.y(), last_orientation_.z(), last_orientation_.w());
  ...
  last_position_.setValue(odom_msg.pose.pose.position.x, odom_msg.pose.pose.position.y, odom_msg.pose.pose.position.z);
