...
OdometrySpoofNode::OdometrySpoofNode() : Node("odometry_spoof") {
    ...
    odom_pub_ =
        this->create_publisher<nav_msgs::msg::Odometry>("odom/filtered", 10);
...
void OdometrySpoofNode::timerCallback() {
    ...
    const std::string target_frame = "imu_link";
    ...
    if (has_last_transform_) {
        ...
        if (dt > 0.0) {
            ...
            // Angular velocity
            tf2::Quaternion q_last(last_orientation_.x(), last_orientation_.y(),
                                   last_orientation_.z(),
                                   last_orientation_.w());
    ...
    last_position_.setValue(odom_msg.pose.pose.position.x,
